...
// -------------------- Telemetry pacing ------------------
static uint32_t lastRtMs = 0;
static uint32_t lastHz1Ms = 0;
...
static void writeAnalyzer(JsonObject data) {
  ...
  if (mode && strcmp(mode, "fft") == 0) {
    ...
    for (uint8_t i = 0; i < bandsLen; ++i) {
      arr.add(static_cast<uint16_t>(bands[i]));
    }
  }
...
static void sendSlowTelemetry(uint32_t now) {
  ...
  if (lastChange == 0) {
    pc["last_change_ms"] = nullptr;
  } else {
    pc["last_change_ms"] = now - lastChange;
  }
...
static void handleAnalyzerJson(JsonObject obj) {
  ...
  if (strcmp(cmd, "set") == 0) {
    if (obj["mode"].is<const char*>()) {
      analyzerSetMode(obj["mode"].as<const char*>());
    }
    if (obj["bands"].is<int>()) {
      analyzerSetBands(static_cast<uint8_t>(obj["bands"].as<int>()));
    }
    if (obj["update_ms"].is<int>()) {
      analyzerSetUpdateMs(static_cast<uint16_t>(obj["update_ms"].as<int>()));
    }
    analyzerSaveToNvs();
...
void commsTick(uint32_t now, bool sqwTick) {
  ...
  if (!shouldSendSlow) {
    if (sqwTick) {
      shouldSendSlow = true;
    } else if (slowInterval == 0 || now - lastHz1Ms >= slowInterval) {
      shouldSendSlow = true;
    }
  }
